...
1. **Operator** loads cluster state from CR. It is mainly to get the list of primaries and replicas.
...
Subpahse 2 - aknoledges await - here **operator** waits for all replicas' okays.  
...
TBD - to make it more clear, also to explain what to implement and how (from ways of blocking). sigterm trap for sidecar is missing. and operator promoting primaries. and DNS issue with headless svc. and status in CRs is missing

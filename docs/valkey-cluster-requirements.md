...
If the certs are provided, on change, it will be validated.
If certs are autogenerated, there will be a job that will regenerate the certs on demand.
Again, in any case, they will be updated in secret from `tlsSecretName` and mounted in pods.

When the secret is changed, the pods must be restarted using *Rolling restarts* (described also in this doc). If CA is the same, nothing must be done, else, a bundle must be provided.
...
TBD - to make it more clear, also to explain what to implement and how (from ways of blocking). sigterm trap for sidecar is missing. and operator promoting masters. and DNS issue with headless svc. and status in CRs is missing
